...
export const argumentsModule = {
  ...
  sync: {
    ...
    insertHook(updateStore, doc, store) {// Check vuex-easy-firestore hooks documentation
  ...
  getters: {
    ...
    argumentBySectionId: (state) => (sectionId) =>
      Object.keys(state.data)
        ...
        .sort((argumentA: ArgumentInterface, argumentB: ArgumentInterface) => argumentA.createdAt.getTime() - argumentB.createdAt.getTime()),

    isUserConvinced: (state, getters, dispatch, rootGetters) => (argument: ArgumentInterface) => {
  ...
  actions: {
     ...
     * @param type
     */
    addArgument: async ({ context, rootGetters, dispatch, commit }, { content, contentHtml, sectionId, type }: { content: string; contentHtml: string; sectionId: string; type: boolean }) => {
      ...
      if (type !== null) { newArgument.type = type; }
      console.log('Dispatch insert newArgument ' + JSON.stringify(newArgument));
      const argId = await dispatch('insert', newArgument);
      const isSectionReachedToThreshold = await dispatch('voteAfterAddingArgument', { sectionId, type });
    ...
    voteAfterAddingArgument: async ({ context, rootGetters, dispatch, commit }, { sectionId, type }) => {
      ...
      if (!rootGetters['votingModule/isUserVoted'](section, vote)) {
        const updatedVotes = await dispatch('votingModule/addVote', { type: 'section', object: section, vote }, { root: true });
        if (rootGetters['votingModule/isSectionReachedToThreshold'](section, updatedVotes)) {
          await dispatch(
    ...
    updateArgumentSectionIds: async ({ state, dispatch }, { oldId, newId }) => {
      ...
      oldIds.map((argument) => {
        dispatch('updateArgument', { id: argument.id, updateObject: { sectionId: newId } });
      });
      newIds.map((argument) => {
        dispatch('updateArgument', { id: argument.id, updateObject: { sectionId: oldId } });
      });
    },
  },
};
